...
class HumanoidRobot(BaseTask):
    ...
    def compute_observations(self):
        ...
        即本体感知
        """
        obs_buf_s = torch.cat((#skill_vector, 
        ...
        if self.cfg.terrain.measure_heights:
            ...
            self.obs_buf = torch.cat([obs_buf_s,heights,obs_buf, heights, priv_explicit, priv_latent, self.obs_history_buf.view(self.num_envs, -1)], dim=-1)
        else:
            self.obs_buf = torch.cat([obs_buf_s,heights,obs_buf, priv_explicit, priv_latent, self.obs_history_buf.view(self.num_envs, -1)], dim=-1)
    ...
    def _update_terrain_curriculum(self, env_ids):
        ...
        dis_to_origin = self.root_states[env_ids, 0] - self.env_origins[env_ids, 0]
        threshold = self.cfg.terrain.terrain_length - self.cfg.terrain.platform_size
        move_up =dis_to_origin > 0.9*threshold
        move_down = dis_to_origin < 0.3*threshold
    ...
    def _get_heights(self, env_ids=None):
        ...
        points_data += self.cfg.terrain.border_size
        points_data = (points_data/self.cfg.terrain.horizontal_scale).long()
        ...
        return heights.view(self.num_envs, -1) * self.cfg.terrain.vertical_scale, heights_data.view(self.num_envs, -1) * self.cfg.terrain.vertical_scale
    ...
    def _reward_feet_parallel(self):
        feet_angles = self.rigid_body_states[:, self.feet_indices, 3:7]  # 提取四元数
        feet_angles = feet_angles.view(-1, 4)
        roll_x, pitch_y, yaw_z = euler_from_quaternion(feet_angles)
        pitch_y = pitch_y.view(-1, 2)
        return torch.sum(torch.square(pitch_y), dim=1)
    
    def _reward_smoothness(self):
        action_diff = self.actions - self.last_actions
        action_diff2 = action_diff - (self.last_actions - self.last_last_actions)
        return torch.norm(action_diff, dim=1) + torch.norm(action_diff2, dim=1)
    ...
    def _reward_self_roll(self):
        return torch.sum(self.roll**2)
    
    def _reward_self_yaw(self):
        return torch.sum(self.yaw**2)
    
    def _reward_huge_step(self):
        feet_pos = self.rigid_body_states[:, self.feet_indices, 0] - self.last_foot_action[:, :, 0]
        if (self.time_stamp == 5) :
            return torch.sum(torch.square(feet_pos))*0.01
        else :
            return 0
    ...
    def _reward_feet_height(self):
        constrain_height = 0.3
        feet_height = self.rigid_body_states[:, self.feet_indices, 2]
        lfeet = torch.mean(feet_height[:,0].unsqueeze(1) - self.measured_heights, dim=1)
        rfeet = torch.mean(feet_height[:,1].unsqueeze(1) - self.measured_heights, dim=1)
        reward = torch.square(lfeet - constrain_height) + torch.square(rfeet - constrain_height)
        return reward
    
    def _reward_goals(self):
        self.level_weights = [3, 5, 7, 9, 11, 13, 15, 17, 19, 20]
        rewards = self.level_weights[self.cur_goal_idx]
        row_rewards = torch.sum(rewards)
        return row_rewards
    
    def _reward_feet_yaw(self):
        feet_angles = self.rigid_body_states[:, self.feet_indices, 3:7]
        feet_angles = feet_angles.view(-1, 4)
        roll_x, pitch_y, yaw_z = euler_from_quaternion(feet_angles)
        yaw_z = yaw_z.view(-1, 2)
        return torch.sum(torch.square(yaw_z), dim=1)
    
    def _reward_knee_yaw(self):
        knee_angles = self.rigid_body_states[:, self.knee_indices, 3:7]
        knee_angles = knee_angles.view(-1, 4)
        roll_x, pitch_y, yaw_z = euler_from_quaternion(knee_angles)
        yaw_z = yaw_z.view(-1, 2)
        return torch.sum(torch.square(yaw_z), dim=1)

    def _reward_knee_height(self):
        knee_height = self.rigid_body_states[:, self.knee_indices, 2]
        constrain_height = 0.5
        rew = (knee_height - constrain_height)**2
        return torch.sum(rew)
    
    def _reward_knee_dis(self):
        dis_limit = 0.3
        knee_tag = self.rigid_body_states[:, self.knee_indices, 1]
        knee_dis = knee_tag[:,0] - knee_tag[:,1]
        return torch.sum(torch.abs(knee_dis-dis_limit))
    
    def _reward_feet_dis(self):
        dis_limit = 0.3
        feet_tag = self.rigid_body_states[:, self.feet_indices, 1]
        feet_dis = feet_tag[:,0] - feet_tag[:,1]
        feet_dis = torch.where(feet_dis < 0, torch.tensor(-2.0, device=feet_dis.device), feet_dis)
        return torch.sum(torch.abs(feet_dis-dis_limit))
    
    def _reward_fallen(self):
        base_height = self.root_states[:, 2]
        feet_height = self.rigid_body_states[:, self.feet_indices, 2]
        feet_low = torch.minimum(feet_height[:,0],feet_height[:,1])
        return torch.square((base_height - feet_low)-self.cfg.rewards.base_height_target)
    
    def _reward_regular_foot(self):
        foot_contact = self.get_foot_contacts()  # [N, 2]
        rew = (foot_contact[:, 0].float() + (~foot_contact[:, 1]).float())
        rew -= (foot_contact[:, 1].float() + (~foot_contact[:, 0]).float())
        sign = torch.where(self.phase > 0.5, 1.0, -1.0)  # [N]
        return torch.sum(rew * sign) / self.num_envs
    
    def _reward_knee_foot(self):
        knee_pos = self.rigid_body_states[:, self.knee_indices, 1]
        feet_pos = self.rigid_body_states[:, self.feet_indices, 1]
        return torch.sum(torch.abs(knee_pos-feet_pos))